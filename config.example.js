...
window.SPEAKX.CONFIG = {
  ...
  SQL: {
    ...
    ab_experiment_summary: "SELECT (idx-1)::int AS e, elem::text AS b, SUM(installs) AS i, SUM(fs_shown) AS fs, SUM(ob_completed) AS ob, SUM(login_users) AS lo, SUM(checkout_load) AS ck, SUM(order_created) AS \"or\", SUM(payments) AS p, ROUND(SUM(payments)*100.0/NULLIF(SUM(installs),0),2) AS cvr FROM d0_conversion_funnel, jsonb_array_elements_text(test_bucket) WITH ORDINALITY AS t(elem, idx) WHERE install_date >= CURRENT_DATE - INTERVAL '90 days' AND test_bucket IS NOT NULL AND elem IN ('A','B','C','CONTROL') GROUP BY 1, 2 ORDER BY 1, 2",

    ab_daily_trend: "SELECT install_date::text AS d, (idx-1)::int AS e, elem::text AS b, ROUND(SUM(payments)*100.0/NULLIF(SUM(installs),0),2) AS c FROM d0_conversion_funnel, jsonb_array_elements_text(test_bucket) WITH ORDINALITY AS t(elem, idx) WHERE install_date >= CURRENT_DATE - INTERVAL '30 days' AND test_bucket IS NOT NULL AND elem IN ('A','B','C','CONTROL') GROUP BY 1, 2, 3 ORDER BY 1, 2, 3",
  },
